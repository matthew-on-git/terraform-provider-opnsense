...
func TestDHCPv6SubnetDataSource_read(t *testing.T) {
	t.Parallel()

	const id = "subnet6-uuid"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet || r.URL.Path != dhcpv6SubnetReqOpts.GetEndpoint+"/"+id {
			t.Fatalf("unexpected request %s %s", r.Method, r.URL.Path)
		}
		_, _ = w.Write([]byte(`{"subnet6":{"subnet":"2001:db8::/64","interface":{"lan":{"value":"LAN","selected":1}},"allocator":{"random":{"value":"Random","selected":1}},"pd-allocator":{"random":{"value":"Random","selected":1}},"pools":"2001:db8::100-2001:db8::1ff","description":"lan6"}}`))
	}))
	t.Cleanup(server.Close)

	client := testClient(t, server.URL)
	ds := newDHCPv6SubnetDataSource()
	configureDataSource(t, ds, client)
	schema := dataSourceSchema(t, ds)
	req := datasource.ReadRequest{Config: modelConfig(t, schema, &DHCPv6SubnetResourceModel{ID: types.StringValue(id)})}
	...
	if state.Subnet.ValueString() != "2001:db8::/64" || state.Interface.ValueString() != "lan" {
