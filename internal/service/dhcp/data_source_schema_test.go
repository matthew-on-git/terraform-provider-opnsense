...
func TestDataSources_schemaIDs(t *testing.T) {
	...
	if len(constructors) != 2 {
		t.Fatalf("expected 2 DHCP data sources, got %d", len(constructors))
	}
...
func TestSubnetDataSource_read(t *testing.T) {
	...
	req := datasource.ReadRequest{Config: modelConfig(t, schema, &SubnetResourceModel{ID: types.StringValue(id)})}
	...
	if state.Subnet.ValueString() != "192.0.2.0/24" || state.Pools.ValueString() == "" {
