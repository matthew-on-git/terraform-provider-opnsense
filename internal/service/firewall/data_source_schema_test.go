...
func TestDataSources_schemaIDs(t *testing.T) {
	...
	if len(constructors) != 6 {
		t.Fatalf("expected 6 firewall data sources, got %d", len(constructors))
	}
...
func TestCategoryDataSource_read(t *testing.T) {
	t.Parallel()

	const id = "category-uuid"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet || r.URL.Path != categoryReqOpts.GetEndpoint+"/"+id {
			t.Fatalf("unexpected request %s %s", r.Method, r.URL.Path)
		}
		_, _ = w.Write([]byte(`{"category":{"name":"automation","auto":{"1":{"selected":1}},"color":"#336699"}}`))
	}))
	...
	config := CategoryResourceModel{ID: types.StringValue(id)}
	...
	if state.Name.ValueString() != "automation" || !state.Auto.ValueBool() || state.Color.ValueString() != "#336699" {
