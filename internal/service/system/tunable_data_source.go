...
func (d *tunableDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = dsschema.Schema{
		MarkdownDescription: "Reads an existing system tunable on OPNsense by UUID.",
		Attributes: map[string]dsschema.Attribute{
			"id":          dsschema.StringAttribute{Required: true, MarkdownDescription: "UUID to look up."},
			"tunable":     dsschema.StringAttribute{Computed: true, MarkdownDescription: "System tunable name."},
			"value":       dsschema.StringAttribute{Computed: true, MarkdownDescription: "Configured tunable value."},
			"description": dsschema.StringAttribute{Computed: true, MarkdownDescription: "Description stored with the tunable."},
		},
	}
}
...
func (d *tunableDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var config TunableResourceModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &config)...)
	if resp.Diagnostics.HasError() {
		return
	}
	id := config.ID.ValueString()
	result, err := opnsense.Get[tunableAPIResponse](ctx, d.client, tunableReqOpts, id)
	if err != nil {
		resp.Diagnostics.AddError("Error reading system tunable", fmt.Sprintf("Could not read system tunable %s: %s", id, err))
		return
	}
