...
// aclReqOpts configures the OPNsense API endpoints for Unbound ACLs.
var aclReqOpts = opnsense.ReqOpts{
	AddEndpoint:         "/api/unbound/settings/add_acl",
	GetEndpoint:         "/api/unbound/settings/get_acl",
	UpdateEndpoint:      "/api/unbound/settings/set_acl",
	DeleteEndpoint:      "/api/unbound/settings/del_acl",
	SearchEndpoint:      "/api/unbound/settings/search_acl",
	ReconfigureEndpoint: "/api/unbound/service/reconfigure",
	Monad:               "acl",
}

// aclResource implements the opnsense_unbound_acl resource.
...
// Create creates a new Unbound ACL via the OPNsense API.
func (r *aclResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Error creating Unbound ACL",
			fmt.Sprintf("Could not create Unbound ACL: %s", err),
		)
		return
	}

	// Read back from API to populate state (never echo from config).
	result, err := opnsense.Get[unboundACLAPIResponse](ctx, r.client, aclReqOpts, uuid)
	if err != nil {
		resp.Diagnostics.AddError(
			"Error reading Unbound ACL after create",
...
func (r *aclResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	result, err := opnsense.Get[unboundACLAPIResponse](ctx, r.client, aclReqOpts, state.ID.ValueString())
	if err != nil {
		...
		resp.Diagnostics.AddError(
			"Error reading Unbound ACL",
			fmt.Sprintf("Could not read Unbound ACL %s: %s", state.ID.ValueString(), err),
		)
		return
	}
...
// Update modifies an existing Unbound ACL via the OPNsense API.
func (r *aclResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Error updating Unbound ACL",
			fmt.Sprintf("Could not update Unbound ACL %s: %s", id, err),
		)
		return
	}

	// Read back from API to populate state (never echo from config).
	result, err := opnsense.Get[unboundACLAPIResponse](ctx, r.client, aclReqOpts, id)
	if err != nil {
		resp.Diagnostics.AddError(
			"Error reading Unbound ACL after update",
...
// Delete removes an Unbound ACL from the OPNsense API.
func (r *aclResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError(
			"Error deleting Unbound ACL",
			fmt.Sprintf("Could not delete Unbound ACL %s: %s", state.ID.ValueString(), err),
		)
	}
}

// ImportState imports an existing Unbound ACL by UUID.
