...
// generalID is the synthetic identifier for this singleton settings object.
const generalID = "unbound-general"
...
func (r *generalResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_unbound_general"
}
...
func (r *generalResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err := opnsense.UpdateSingleton(ctx, r.client, generalReqOpts, plan.toAPI(ctx)); err != nil {
		resp.Diagnostics.AddError("Error applying the Unbound DNS general settings", fmt.Sprintf("%s", err))
		return
	}
	result, err := opnsense.GetSingleton[generalAPIResponse](ctx, r.client, generalReqOpts)
	if err != nil {
		resp.Diagnostics.AddError("Error reading the Unbound DNS general settings after apply", fmt.Sprintf("%s", err))
		return
	}
...
func (r *generalResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Error reading the Unbound DNS general settings", fmt.Sprintf("%s", err))
		return
	}
...
func (r *generalResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err := opnsense.UpdateSingleton(ctx, r.client, generalReqOpts, plan.toAPI(ctx)); err != nil {
		resp.Diagnostics.AddError("Error updating the Unbound DNS general settings", fmt.Sprintf("%s", err))
		return
	}
	result, err := opnsense.GetSingleton[generalAPIResponse](ctx, r.client, generalReqOpts)
	if err != nil {
		resp.Diagnostics.AddError("Error reading the Unbound DNS general settings after update", fmt.Sprintf("%s", err))
		return
	}
	plan.fromAPI(ctx, result, generalID)
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
