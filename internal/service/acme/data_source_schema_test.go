...
func TestDataSources_schemaIDs(t *testing.T) {
	...
	if len(constructors) != 3 {
		t.Fatalf("expected 3 ACME data sources, got %d", len(constructors))
	}
...
func TestAccountDataSource_read(t *testing.T) {
	...
	const id = "acme-account-uuid"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet || r.URL.Path != accountReqOpts.GetEndpoint+"/"+id {
			t.Fatalf("unexpected request %s %s", r.Method, r.URL.Path)
		}
		_, _ = w.Write([]byte(`{"account":{"enabled":"1","name":"prod","description":"account","email":"admin@example.com","ca":{"letsencrypt":{"value":"Let's Encrypt","selected":1}}}}`))
	}))
	...
	req := datasource.ReadRequest{Config: modelConfig(t, schema, &AccountResourceModel{ID: types.StringValue(id)})}
	...
	if state.Name.ValueString() != "prod" || state.CA.ValueString() != "letsencrypt" || !state.Enabled.ValueBool() {
