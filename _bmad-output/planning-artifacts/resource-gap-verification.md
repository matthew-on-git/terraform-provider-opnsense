...
| Interface LAGG | Coming with live validation gate | Published OPNsense API docs expose `interfaces/lagg_settings` item CRUD/search/reconfigure endpoints and `Lagg.xml`. Repository schema has no generated `interface_lagg` resource. | Implement `opnsense_interface_lagg` only after live appliance validation confirms selectable member interface behavior. |
...
| System tunables / sysctl | Coming with safety/live-validation gate | Story 28.3 confirmed current OPNsense docs/source expose persistent `core/tunables` item CRUD/search plus `reconfigure`; model `Tunables.xml` mounts at `//sysctl` and stores `item` entries. | Implement only after live validation confirms CRUD/reconfigure/import behavior and safety documentation for high-risk kernel/network tunables. |
...
| System tunables / sysctl | `POST /api/core/tunables/add_item`, `GET /api/core/tunables/get_item/{uuid?}`, `POST /api/core/tunables/set_item/{uuid}`, `POST /api/core/tunables/del_item/{uuid}`, `GET,POST /api/core/tunables/search_item`, `POST /api/core/tunables/reconfigure`, `POST /api/core/tunables/reset` | Published core API docs and `TunablesController.php` confirm item CRUD/search; `reconfigure` restarts `login` and `sysctl`; `reset` restores factory tunables and should not be used for normal resource lifecycle. | `sysctl` for add/get/set item; model path `item`. | Coming; UUID item resource candidate with safety/live-validation gate. |
